...
export function buildTree(files: TreeInput[]): TreeNode[] {
  ...
  // Merge nested dirs with same prefix
  function merge(nodes: TreeNode[]): TreeNode[] {
    return nodes.map((node) => {
      if (!node.isDir) return node;
      const merged = merge(node.children);
      // If dir has exactly one child which is also a dir, merge names
      if (merged.length === 1 && merged[0]!.isDir) {
        return {
          ...node,
          name: `${node.name}/${merged[0]!.name}`,
